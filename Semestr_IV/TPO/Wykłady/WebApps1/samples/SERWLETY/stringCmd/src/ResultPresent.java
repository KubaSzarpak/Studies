...
public class ResultPresent extends HttpServlet {
                             ...
                             throws ServletException, IOException
  {
    ...
    /* Jakie by³y parametry Command i ich wartoœci
    String[] pnames = BundleInfo.getCommandParamNames();
    String[] pdescr = BundleInfo.getCommandParamDescr();
    for (int i=0; i<pnames.length; i++) {
      String pval = (String) context.getAttribute("param_"+pnames[i]);
      out.println(pdescr[i] + ": = \"" + pval + "\"<br>");
    }
    */
